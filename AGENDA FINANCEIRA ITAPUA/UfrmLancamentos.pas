...
begin
  ...
  begin
    ...
    // Parcelando faz parte do botão gerar parcelas, que precisa excluir
...
var
  valorpago: Currency;
begin
  if MessageBox(Application.Handle, 'Todos os dados exibidos serão pagos' + #13
    + 'Esta operação é irreversível.', 'Atenção', MB_YESNO + MB_ICONQUESTION) = mrYes
  then
  begin
    FDqryLcto.First;
...
begin
  ...
  begin
    ...
    // parâmetro carregado com o valor do checkbox "cbPago" (status). Essa variável traz o valor atribuido no evento do btnPesquisaPrevisto.
  ...
  begin
    try
      ...
      begin
        ...
        ShowMessage('Impossível excluir este lançamento em razão do motivo: '
          + auxErro);
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      qryLogin.Open();
      MessageDlg('Não há permissão para editar esse lançamento.' + #13 + #13 +
        'Apenas os usuários que fizeram o lançamento podem alterá-los ou excluí-los.'
...
begin
  ...
  // variável carregada com a função que verifica se há campos vazios.
...
begin
  if MessageBox(Application.Handle,
    'Deseja realmente gerar parcelas para este lançamento?   ', 'Confirmação',
  ...
  begin
    ...
    frmConfirmarParcelamentos.MemoInform.Text := 'Serão geradas ' + #13 + #13 +
