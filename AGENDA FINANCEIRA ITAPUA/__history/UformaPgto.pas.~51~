...
begin
  if MessageBox(Application.Handle,'Deseja realmente alterar esta forma de pagamento','Confirmação',MB_YESNO + MB_ICONQUESTION) = mrYes then
     begin
      FDqryFormapgto.Edit;
      configurarEnables(1);
...
procedure TfrmFormapgto.btnExcluirClick(Sender: TObject);
  var auxErro : string;
begin
 if MessageBox(Application.Handle,'Deseja realmente excluir esta forma de pagamento?','Confirmação', MB_YESNO + MB_ICONQUESTION) = mrYes then
    begin
      try
        FDqryFormapgto.Delete;
         ...
         begin
          ...
          +#13+ 'Para mais informações, pesquise sobre a ocorrência: ' + AuxErro, TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
         end;
      end;
    end;
end;
