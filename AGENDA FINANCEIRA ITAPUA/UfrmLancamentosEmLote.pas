...
begin
  ...
  begin
    ...
    if ValorCelula = False then
      ImagemIndex := 0
      // Substitua pelo índice da imagem ativa no seu TPngImageList
    else
      ImagemIndex := 1;
    // Substitua pelo índice da imagem inativa no seu TPngImageList
...
begin
  if MessageBox(Application.Handle, 'Confirma a exclusão dos dados carregados?'
    + #13 + 'Todas as alterações realizadas serão perdidas', 'Confirmação',
...
begin
  ...
  begin
    MessageDlg('Não há lote carregado para o lançamento',
  ...
  if MessageBox(Application.Handle, 'Confirma o lançamento do lote carregado?',
...
const
  COM_ACENTO: array [0 .. 44] of string = ('ä', 'à', 'â', 'ë', 'ê', 'ô', 'û',
    'ã', 'õ', 'á', 'è', 'é', 'î', 'ï', 'ì', 'í', 'ö', 'ò', 'ó', 'ù', 'ú', 'ç',
    'ü', 'ñ', 'À', 'Â', 'Ê', 'Ô', 'Û', 'Ã', 'Õ', 'À', 'Á', 'È', 'É', 'Ì', 'Í',
    'Ò', 'Ó', 'Ù', 'Ú', 'Ç', 'Ü', '€', 'Ñ');
