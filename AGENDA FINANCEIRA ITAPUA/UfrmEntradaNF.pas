...
begin
   ...
   if ProdutoZerado <> '' then
       begin
         MessageDlg('Não foi possível excluir a Entrada. '+ #13 + 'É necessário verificar o saldo do(s) produto(s) abaixo: ' + #13 + #13 + ' ' + ProdutoZerado, TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
         Abort;
       end;
end;
...
begin
    ...
    begin
       ...
       FDqryMovProduto['ADICIONAIS'] := 'Ajuste de estoque pela exclusão de NF [Entrada]';
       FDqryMovProduto['OBS'] := 'Exclusão da Entrada NF: '+ IntToStr(FDqryEntradaNF['NF']) +' | Entrada '+ IntToStr(FDqryEntradaNF['CADASTRO_NF_ID']) + ' | User: ' + frmPrincipal.FDqryLogin['NOME'] + '.';
    end;
...
begin
    ...
    begin
      if FDqryEntradaNF['STATUS'] = 'NF ABERTA' then
       ...
       MessageDlg('Não é possível efetuar alterações em entradas já processadas', TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
    end
  else
  MessageDlg('Não há Entrada selecionada para adicionar itens', TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
end;

procedure TfrmEntradaNF.AtualizarEstoqueDeProdutos;
 var AtualizarValorEstoque : string;
begin
 if MessageBox(Application.Handle,'Deseja atualizar o valor dos produtos'+#13+'com os valores desta entrada?','Confirmação',MB_YESNO + MB_ICONQUESTION) = mrYes then
    AtualizarValorEstoque := 'SIM'
   else
    AtualizarValorEstoque := 'NÃO';
  while not FDqryItensNF.Eof do
   begin
...
procedure TfrmEntradaNF.btnSalvarClick(Sender: TObject);
 begin
         VerificarCamposVazios; //procedure que verifica campos vazios e aborta a operação caso haja.
         EnableDisable(0);
...
var AuxErro : string;
begin
   if MessageBox(Application.Handle,'Atenção! '+#13 +'Essa operação excluirá a entrada selecionada e todos os itens que foram cadastrados nela.' +#13 + 'Deseja continuar?','Confirmação', MB_YESNO + MB_ICONQUESTION) = mrYes then
    begin
         // if FDqryEntradaNF['STATUS'] = 'PROCESSADA' then //se aberta, não fez alteração de estoque, portanto, não precisa verificar estoque.
         //   VerificarEstoqueZerado;  //proibição de estoque negativo - impede exclusão se o saldo do produto for menor do que a qtd a ser excluída.
      if FDqryEntradaNF['NF_LANCADA'] = 'SIM' then
         begin
           MessageDlg('Não há permissão para excluir essa Entrada.' + #13 +'Ela já foi cadastrada em um lançamento financeiro.' +#13+ #13+'Para excluí-la, é necessário deletar o lançamento no registro financeiro.  ', TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
           Abort;
         end;
     begin
       try
         FDqryItensNF.First;
         while not FDqryItensNF.Eof do   //Trecho abaixo retirado para não haver tentativa de atualização do estoque do produto. Apenas os itens da NF serão excluiídos.
            begin
             {if FDqryEntradaNF['STATUS'] = 'PROCESSADA' then
              begin
                FDqryProduto.Edit;  //Não é necessário locate no produto em razão da FDqryProduto ser filha no MASTERDATA da FDqryItensNF
                FDqryProduto['ESTOQUE'] := FDqryProduto['ESTOQUE'] - FDqryItensNF['QTDE_ITEM'];
        ...
        on E : exception do
         begin
          ...
          MessageDlg('Não é possível excluir o registro no momento ' + AuxErro, TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
         end;
       end;
     end;
      ProcedureCommitOrRollbackRetaining;
  ...
  // if (rbEntrada.Checked = True) or (rbAjuste.Checked = True) then
   //   begin
         if (FDqryEntradaNF.RecordCount > 0) and (FDqryItensNF.RecordCount > 0) and (FDqryEntradaNF['STATUS'] = 'NF ABERTA') then
            begin
               if MessageBox(Application.Handle,'Atenção! '+#13+'Esta operação não poderá ser desfeita. Deseja continuar?','Confirmação', MB_YESNO + MB_ICONQUESTION) = mrYes then
                    begin
                  ...
                  MessageDlg('Não há Entrada/Itens ou a NF já foi processada', TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
   //   end
  // else
  //    MessageDlg('Por favor, escolha o parâmetro da entrada ("Entrada Normal ou Ajuste de Estoque") ', TMsgDlgType.mtWarning,[TMsgDlgBtn.mbOK],0);
 end;
