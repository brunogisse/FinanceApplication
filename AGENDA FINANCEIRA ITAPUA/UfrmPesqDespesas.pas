    ...
    begin
      with FDqrySubdespesa do
        begin
         Close;
         SQL.Clear;
         SQL.Add('select S.SUBCATEGORIA_ID, S.DESCRICAO,S.VALOR_MAXIMO, S.CATEGORIA_ID as CATEGORIA_FK, C.CATEGORIA_ID, C.DESCRICAO as CATEGORIA from SUBCATEGORIA S, CATEGORIA C where (S.CATEGORIA_ID = C.CATEGORIA_ID) and (S.DESCRICAO LIKE '+ QuotedStr('%'+editPesquisa.Text+'%')+ ') order by C.DESCRICAO');
         Open;
        end;
    end;
    ...
    begin
      with FDqrySubdespesa do
        begin
         Close;
         SQL.Clear;
         SQL.Add('select S.SUBCATEGORIA_ID, S.DESCRICAO,S.VALOR_MAXIMO, S.CATEGORIA_ID as CATEGORIA_FK, C.CATEGORIA_ID, C.DESCRICAO as CATEGORIA from SUBCATEGORIA S, CATEGORIA C where (S.CATEGORIA_ID = C.CATEGORIA_ID) and (S.DESCRICAO LIKE '+ QuotedStr('%'+editPesquisa.Text+'%')+ ') order by S.DESCRICAO');
         Open;
        end;
    end;
...
procedure TfrmPesqDespSub.enviarSubdespesa;
begin
  if setarEditFoco = 'relatório' then
   ...
   begin
     frmLancamento.editPesquisaDespSub.Text:= FDqrySubdespesa['DESCRICAO'] + '  [' + FDqrySubdespesa['CATEGORIA'] + ']';
      ...
      with gridPesqDespSub do
       begin
         if DataSource.DataSet.State in [dsEdit, dsInsert, dsBrowse] then //Cor da linha selecionada
           if (Rect.Top = TStringGrid(gridPesqDespSub).CellRect( DataCol ,TStringGrid(gridPesqDespSub).Row).Top)
