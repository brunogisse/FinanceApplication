...
type
  TfrmCategoriaGeral = class(TForm)
    ...
    procedure gridSubcategoriaDrawColumnCell(Sender: TObject; const Rect: TRect;
      DataCol: Integer; Column: TColumn; State: TGridDrawState);
  private
...
begin
       ...
       begin
         if DataSource.DataSet.State in [dsEdit, dsInsert, dsBrowse] then //Cor da linha selecionada
           if (Rect.Top = TStringGrid(gridCategoria).CellRect( DataCol ,TStringGrid(gridCategoria).Row).Top)
...
begin
       ...
       begin
         if DataSource.DataSet.State in [dsEdit, dsInsert, dsBrowse] then //Cor da linha selecionada
           if (Rect.Top = TStringGrid(gridSubcategoria).CellRect( DataCol ,TStringGrid(gridSubcategoria).Row).Top)
             or( gdSelected in State)  then
            begin
              Canvas.FillRect(Rect);
              Canvas.Brush.Color := clNavy;
